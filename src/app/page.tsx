      ...
      return new Promise((resolve) => {
        ...
        if (Hls.isSupported()) {
          const hls = new Hls({ 
            ...
            xhrSetup: (xhr) => {
              // Custom headers for better stability if needed
            }
          });
          hlsRef.current = hls;
          hls.loadSource(url);
            ...
            {showSettings && (
              <div className="absolute right-0 top-full mt-3 w-72 bg-zinc-900/95 border border-white/10 rounded-3xl shadow-2xl z-[100] overflow-hidden backdrop-blur-2xl ring-1 ring-white/10">
                <div className="p-5 border-b border-white/5 flex items-center justify-between">
                  <span className="text-xs font-black text-zinc-500 uppercase tracking-[2px]">Playlist sources</span>
          ...
          <div className="max-w-6xl mx-auto p-4 md:p-10 space-y-10">
              ...
              <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 px-4">
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    {currentLevel !== -1 && (
                      <span className="px-3 py-1 rounded-full bg-green-500/10 text-green-500 text-[10px] font-black uppercase tracking-[2px] border border-green-500/20">
                        {levels[currentLevel]?.height}P HD
                      </span>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
              <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 hover:border-indigo-500/30 transition-all group relative overflow-hidden">
                ...
                <History className="text-blue-500 mb-5 group-hover:scale-110 transition-transform" size={28} />
                <h4 className="font-black text-sm uppercase tracking-widest mb-3">Watch History</h4>
                <p className="text-[11px] text-zinc-500 leading-relaxed font-medium">Lacak 15 channel terakhir yang Anda putar untuk akses instan kembali.</p>
              </div>
